                        ...
                        <div class="table-wrap mt-40">
                            <div class="table-responsive">
                                <table id="deposit_pending" class="table mb-0">
                                    ...
                                    <tbody>
                                        @foreach ($user_wallet as $ks=> $vs)                                        
                                            <tr>
                                                <td>{{$ks+1}}</td>
                                                <td>{{$vs->tr_value}}</td>
                                                <td>{{$vs->type}}</td>
                                                <td>{{$vs->transaction_id}}</td>
                                                <td>{{$vs->tr_remark}}</td>
                                                @if($vs->tr_status == "Pending")
                                                <td style="color:red">{{$vs->tr_status}}</td>
                                                @else
                                                <td style="color:green">{{$vs->tr_status}}</td>
                                                @endif
                                                <td>{{$vs->created_at}}</td>
                                            </tr>
                                        @endforeach
...
@push('scripts')
    ...
    <script>
        function DepositSuccess() {
            ...
            $.ajax({
                url: "{{URL::to('administrator/deposit-approve')}}", 
                method: "POST",
                headers: {
                    'X-CSRF-TOKEN': token
                },
    ...
    <script>
        function DepositCancel() {
            ...
            var token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
            $.ajax({
                url: "{{URL::to('administrator/deposit-cancelled')}}", 
                method: "POST",
                headers: {
                    'X-CSRF-TOKEN': token
                },
