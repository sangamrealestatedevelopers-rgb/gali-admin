...
class DepositManagementOldController extends Controller
{
    ...
    {
        return view('administrator.deposit_management_old.pending');
    }
    public function deposit_success()
    {
        return view('administrator.deposit_management_old.success');
    }

    public function deposit_old()
    {
        return view('administrator.deposit_management_old.deposit_old');
    }

    public function deposit_cancelled()
    {
        return view('administrator.deposit_management_old.cancelled');
    }
    public function deposit_dateway_pending()
    {
        return view('administrator.deposit_management_old.datewaypending');
    }
    ...
    public function DateDepositManageData(Request $request)
    {
        ...
        foreach ($items as $item) {
            ...
            $widthraw_amount = Helper::depositamout_old($date);
            $nestedData[] = $widthraw_amount;
            $widthraw_amount_count = Helper::depositamout_count_old($date);
            ...
            $ViewLink = '<a href="' . url('/administrator/game/deposit-success-view-old/' . date("Y-m-d", strtotime($item->date))) . ' " title="View"><button class="btn btn-primary">View</button></a>';
            $nestedData[] = $ViewLink;
    ...
    public function deposit_success_view($date)
    {
        ...
        return view('administrator.deposit_management_old.view', compact('select'));
    }
    public function depositManageData_pending(Request $request)
    {
        ...
        $totalFiltered = $totalItems;
        $items = PointTable::where('tr_nature', 'TRDEPO002')->orderBy('created_at', 'desc')->where('tr_status', 'Pending');
        if ($requestData['search']['value']) {
            $filtered = $requestData['search']['value'];
            $items = $items->whereHas('user_data', function ($q) use ($filtered) {
                $q->where('FullName', 'Like', '%' . $filtered . '%')->orwhere('mob', 'Like', '%' . $filtered . '%');
        ...
        foreach ($items as $item) {
            ...
            $editpending_data = '<a href="' . url('/administrator/edit-deposit-pending-data-old/' . $item->id) . '" title="Edit"><button class="btn btn-warning ">Edit</button></a>';
            $editLink = '<a href="' . url('/administrator/deposit-approve-old/' . $item->id) . '" title="Approve"><button class="btn btn-warning ">Approve</button></a>';
            // $deleteLink = '<a href="' . url('/administrator/delete/' . $item->id) . ' " onclick="return confirm("Are you sure want to delete?")" title="Delete"><button class="btn btn-info "><i class="icon-trash"></i></button></a>';
            $ViewLink = '<a href="' . url('/administrator/deposit-cancelled-old/' . $item->id) . ' " title="Cancelled"><button class="btn btn-primary">Cancelled</button></a>';
    ...
    {
        $dta = PointTable::where('id', $id)->first();
        ...
        PointTable::where('id', $id)->update(['tr_status' => 'Cancelled']);
    ...
    public function deposit_approve(Request $request)
    {
        ...
        foreach ($user as $vs) {
            $point = PointTable::where('id', $vs)->first();
            ...
            PointTable::where('id', $vs)->update(['tr_status' => 'Success']);
        }

        return redirect('administrator/game/deposit-pending-old');
    }
    public function deposit_cancel(Request $request)
    {
        ...
        $user = $request->userIds;
        foreach ($user as $vs) {
            PointTable::where('id', $vs)->update(['tr_status' => 'Cancelled']);
        }
    ...
    public function depositManageData_success(Request $request)
    {
        ...
        foreach ($items as $item) {
            ...
            // $deleteLink = '<a href="' . url('/administrator/delete/' . $item->id) . ' " onclick="return confirm("Are you sure want to delete?")" title="Delete"><button class="btn btn-info "><i class="icon-trash"></i></button></a>';
            $candelled = '<a href="' . url('/administrator/success-deposit-cancelled-old/' . $item->id) . ' " title="Cancelled"><button class="btn btn-primary">Cancelled</button></a>';
    ...
    public function depositManageData_cancelled(Request $request)
    {
        ...
        foreach ($items as $item) {
            ...
            $approve = '<a href="' . url('/administrator/deposit-approve-old/' . $item->id) . '" title="Approve"><button class="btn btn-warning ">Approve</button></a>';
            // $nestedData[] = $approve;
            // $nestedData[] = $ViewLink . " " . $editLink . "  " . $deleteLink . " " . $result . " " . $history;
            $datas[] = $nestedData;
        }
    ...
    {
        $point = PointTable::where('id', $id)->first();
        return view('administrator.deposit_management_old.edit',compact('point'));
    }

    public function Update_Deposit_Pending_Data(Request $request)
    {
        $response = PointTable::where('id', $request->id)->update(['tr_value' => $request->amount]);
        if($response)
        {
            return redirect('administrator/game/deposit-pending-old');
        }else{
