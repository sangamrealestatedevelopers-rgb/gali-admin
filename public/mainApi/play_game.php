...
/*$tblCode="DISAWAR";
if($tblCode=="DISAWAR" or $tblCode=="KDMNIGHT")
{
...
foreach ($bets as $key => $value) {
	...
	if ($value->betamount < 1) {
		$rows['success'] = '3';
		$rows['message'] = 'Min Rs 2 Bat';
...
function check_time($market_id)
{
	...
	if ($count > 0) {
		...
		$betCloseSec = $remainingTimeSec - 0; // bet placing closed time
	...
	if ($count_chk_user > 0) {
		...
		if ($tblCode == '') {
	       ...
	       VALUES ('$betType', '$appId', 'ADMIN0001', '$user', '$tr_id', 'TRGAME001', '$betamount', '$newbalance', 'Game', 'Debit', '$dev_model', '$date', '$date_time', '', 'Success', '$tblCode', '$timeId', '$betkey', '0', '','$dev_id','$date','$date_time','mainMarket') ";
					...
					//   echo $sql;
					// die('ppp');
					$sql1 = "INSERT INTO `game_load`(`game_type`,`user_id`, `transaction_id`,`app_id`,`tr_nature`, `tr_value`,  `date`, `date_time`,`tr_status`, `table_id`, `pred_num`,`created_at`)
	       VALUES ('$betType','$user','$tr_id', '$appId','TRGAME001', '$betamount','$date', '$date_time', 'Success', '$tblCode', '$betkey', '$date_time') ";
			...
			$datacommission = "SELECT * FROM manage_commissions WHERE date='" . $date . "' AND market_id='" . $tblCode . "' AND user_id='" . $user . "'";
			$resultget = mysqli_query($conn, $datacommission);
			$dataobj = mysqli_fetch_assoc($resultget);
			if (count($dataobj) > 0) {
				$amount1 = $dataobj['bat_amount'] + $amount;
				$dataupdate = "UPDATE manage_commissions SET bat_amount='" . $amount1 . "' WHERE id='" . $dataobj['id'] . "'";
				$resultupdate = mysqli_query($conn, $dataupdate);

			} else {
				$managecommission = "INSERT INTO `manage_commissions`(`user_id`, `market_id`, `bat_amount`, `win_amount`, `status`, `date`) VALUES ('$user','$tblCode','$amount','$win_amount','$status','$date')";
				$result = mysqli_query($conn, $managecommission);
			}
