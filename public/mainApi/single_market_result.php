 ...
 $count = mysqli_num_rows($sqluser);
  ...
  $marketData = mysqli_fetch_assoc($allMarkets);
     ...
     if($type == '1'){
            ...
            $getResult = mysqli_query($conn, "SELECT * FROM results_tbls WHERE app_id = '".$appId."' AND market_id = '$market_id' order by id desc limit 30 order by id desc");

     }else if($type == '2'){
        
                  if($from != '' || $to == ''){
                                         $getResult = mysqli_query($conn, "SELECT * FROM results_tbls WHERE app_id = '".$appId."' AND market_id = '$market_id' AND date = '$from' order by id desc");
