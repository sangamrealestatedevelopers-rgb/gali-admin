...
function get_win($tid)
{
	global $conn;
	$s = mysqli_query($conn, "SELECT * FROM `point_table` WHERE  transaction_id='" . $tid . "' and is_result_declared='0'");
	$data = mysqli_fetch_assoc($s);
	return $data['tr_value'];
}
...
if ($appId == '' ||  $usrId == '') {
	...
	if ($count_chk_user > 0) {
		if ($type == 'regular') {
			$sqler = mysqli_query($conn, "SELECT * FROM `point_table` WHERE  app_id = '" . $appId . "' AND user_id = '" . $usrId . "' AND is_deleted = '0' AND tr_nature = 'TRWIN005' AND status != 'starline' ORDER BY id DESC");
		...
		if ($count > 0) {
			...
			while ($row2 = mysqli_fetch_assoc($sqler)) {
				...
				$row2['win_value'] = get_win($row2['transaction_id']);
				$rows['success'] = "1";
