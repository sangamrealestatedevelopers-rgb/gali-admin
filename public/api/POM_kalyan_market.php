...
if ($user_id == '' && $appId == '' && $dev_id == '') {
     ...
     if ($count > 0) {
          $allMarkets = mysqli_query($conn, "SELECT * FROM comx_appmarkets WHERE app_id = '" . $appId . "' AND market_type = '2' order by market_position asc ");

          $countMarkets = mysqli_num_rows($allMarkets);

          if ($countMarkets > 0) {
               $rows['status'] = "1";
               $rows['message'] = "Markets Fetched Successfully ";
