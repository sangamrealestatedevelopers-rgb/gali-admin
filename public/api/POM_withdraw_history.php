...
if ($appId == '' || $devId == '' || $usrId == '') {
	...
	if ($count_chk_user > 0) {
		$sqler = mysqli_query($conn, "SELECT * FROM `point_table` WHERE  app_id = '" . $appId . "' AND user_id = '" . $usrId . "' AND is_deleted = '0' AND tr_nature IN('TRWITH003')  ORDER BY id DESC");
		...
		if ($count > 0) {
			...
			while ($row2 = mysqli_fetch_assoc($sqler)) {
				...
				$rows['data'][$i]['date_time'] = date('d-m-Y H:i:s', strtotime($row2['created_at']));
