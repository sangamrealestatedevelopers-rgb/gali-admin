...
if ($appId == '' || $devId == '' || $usrId == '') {
	...
	if ($count_chk_user > 0) {
		...
		if ($dataType == '1') {
			$sqler = mysqli_query($conn, "SELECT * FROM `point_table` WHERE  app_id = '" . $appId . "'  AND user_id = '" . $usrId . "' AND is_deleted = '0' AND tr_nature !='TRGAME001' AND tr_status = 'Success' ORDER BY id DESC");
		} elseif ($dataType == '2') {
			if (!$fltDate == '' && $tblCode == '') {
				$sqler = mysqli_query($conn, "SELECT * FROM `point_table` WHERE  app_id = '" . $appId . "' AND user_id = '" . $usrId . "' AND is_deleted = '0' AND date = '$fltDate' AND c = 'TRGAME001' ORDER BY id DESC");
				...
				$sqler = mysqli_query($conn, "SELECT * FROM `point_table` WHERE  app_id = '" . $appId . "' AND user_id = '" . $usrId . "'  AND is_deleted = '0' AND date = '$fltDate' AND tr_nature = 'TRGAME001' ORDER BY id DESC");
			} else {
				$sqler = mysqli_query($conn, "SELECT * FROM `point_table` WHERE  app_id = '" . $appId . "' AND user_id = '" . $usrId . "' AND is_deleted = '0' AND tr_nature = 'TRGAME001' ORDER BY id DESC");
			}

		} elseif ($dataType == '3') {
			$sqler = mysqli_query($conn, "SELECT * FROM `point_table` WHERE  app_id = '" . $appId . "'  AND user_id = '" . $usrId . "' AND is_deleted = '0' AND tr_nature !='TRGAME001' AND tr_status = 'Pending' ORDER BY id DESC");
		}
		...
		if ($count > 0) {
			...
			while ($row2 = mysqli_fetch_assoc($sqler)) {
				...
				$tblcodeid = mysqli_query($conn, "SELECT * FROM `tbl_types` WHERE  tbl_code = '" . $table_id . "'");
