...
$countpid = mysqli_query($conn, "SELECT * FROM `play_game_ids` WHERE  userid = '" . $user101 . "' AND pid = '".$pid101."'");
	$countpidcount = mysqli_num_rows($countpid);
	if($countpidcount > 0){
		$rows['success'] = '6';
		$rows['message'] = "Sorry";
		echo (json_encode($rows));
		...
		$sql12001 = "INSERT INTO `play_game_ids`(`userid`,`pid`) VALUES ('$user101','$pid101') ";
				mysqli_query($conn, $sql12001);
	}
...
function check_time($market_id)
{
	...
	if ($count > 0) {
		...
		if ($openTimeSec <= $currentTimeSec && $closeTimeSec > $currentTimeSec && $betCloseSec > 0) {
			...
			if ((!(strtotime($ctime) >= strtotime("00:40")) or !(strtotime($ctime) <= strtotime("07:00")))) {
				if (strtotime($ctime) > strtotime("07:00") and strtotime($ctime) <= strtotime('23:59')) {
					$rangeEnd = strtotime("tomorrow 01:00 am") - strtotime($ctime1);
				} else {
					$rangeEnd = strtotime("today 00:40 am") - strtotime($ctime1);
				}
...
function check_timeOneHours($market_id)
{
	$timestamp = time();
	$date = date("Y-m-d", $timestamp);
	date_default_timezone_set('Asia/Kolkata');
	...
	if ($count > 0) {
		...
		if ($openTimeSec <= $currentTimeSec && $closeTimeSec > $currentTimeSec && $betCloseSec > 0) {
			...
			return $rows['remaining_time_in_seconds'];
			...
			if ((!(strtotime($ctime) >= strtotime("03:30")) or !(strtotime($ctime) <= strtotime("09:00")))) {
				if (strtotime($ctime) > strtotime("09:00") and strtotime($ctime) <= strtotime('23:59')) {
					$rangeEnd = strtotime("tomorrow 12:00 am") - strtotime($ctime1);
				} else {
					$rangeEnd = strtotime("today 12:00 am") - strtotime($ctime1);
				}
				if ($rangeEnd != 0) {
					$rows['remaining_time_in_seconds'] = $rangeEnd;
					 return $rows['remaining_time_in_seconds'];
...
if ($appId == '' || $dev_id == '' || $user == '') {
	...
	if ($count_chk_user > 0) {
		...
		$sqler = mysqli_query($conn, "SELECT * FROM `comx_appmarkets` WHERE  market_id = '" . $tblCode . "' ORDER BY market_position = 'ASC'");
		$row2 = mysqli_fetch_assoc($sqler);
		$marketBeforeCloseTime = date("H:i", strtotime($row2['market_view_time_close'] . " -1 hour"));
		$currentTime = date("H:i A");
		$marketBeforeCloseTimeSec = strtotime($marketBeforeCloseTime);
		$currentTimeSec = strtotime($currentTime);
		$marketclosetime = date("H:i", strtotime($row2['market_view_time_close']));

		$chk = check_timeOneHours($row2['market_id']);
		$minusone = $chk - 3660;
		if ($minusone < 0) {
			if($value['bettype'] == "9" || $value['bettype'] == "10"){
				if($value['betamount'] > 300){
					$rows['success'] = '6';
						$rows['message'] = "Maximum Bet Amount 300 RS.";
						echo (json_encode($rows));
						$conn->close();
						exit;
				}
			}else{
				if($value['betamount'] > 100){
					$rows['success'] = '6';
						$rows['message'] = "Maximum Bet Amount 100 RS.";
						echo (json_encode($rows));
						$conn->close();
						exit;
				}
		...
		//echo "SELECT credit,win_amount  FROM us_reg_tbl WHERE user_id ='" . $user . "' AND app_id = '" . $appId . "'"; 
